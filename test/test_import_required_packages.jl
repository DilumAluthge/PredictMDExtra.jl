...
Test.@test( isa(DataFramesMeta, Module) )

import DecisionTree
Test.@test( isa(DecisionTree, Module) )

import DefaultApplication
Test.@test( isa(DefaultApplication, Module) )

import Documenter
Test.@test( isa(Documenter, Module) )

import FileIO
Test.@test( isa(FileIO, Module) )
...
Test.@test( isa(IterableTables, Module) )

import IndirectArrays
Test.@test( isa(IndirectArrays, Module) )
...
import NamedArrays
Test.@test( isa(NamedArrays, Module) )
...
Test.@test( isa(SplitApplyCombine, Module) )
...
import TextParse
Test.@test( isa(TextParse, Module) )
